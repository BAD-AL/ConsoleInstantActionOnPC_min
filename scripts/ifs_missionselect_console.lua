...
-- }
--[[
local current_map_list = missionselect_listbox_contents
instant_action_map_lists = {
	default = missionselect_listbox_contents,
	no_space = {
...
function get_iState(num)
	if(num == 0) then return "map" end
	if(num == 1) then return "mode" end
	if(num == 2) then return "era" end
	if(num == 3) then return "command" end
	if(num == 4) then return "playlist" end
...
-- 	},
	...
	Enter = function(this, bFwd)
		print("ifs_missionselect_console: Enter")
...
-- 		end
		...
		print("ifs_missionselect_console: iState: " .. get_iState(this.iState))
	...
	Input_Accept = function(this)
		...
		if(this.iState == ifs_ms_state.map) then 
			gMouseListBox = this.ifs_ms_MapList_layout
		elseif(this.iState == ifs_ms_state.mode) then 
			gMouseListBox = this.ifs_ms_ModeList_layout
		elseif(this.iState == ifs_ms_state.era) then 
			gMouseListBox = this.ifs_ms_EraList_layout
		end
...
-- 		end -- column == 3
	...
	Input_KeyDown = function(this, iKey)
		...
		if(iKey == 27 or iKey == 8 --[[Esc, Backspace ]]) then
			this.Input_Back(this)
		elseif(iKey == 33 --[[PageUp]]) then
			this.Input_LTrigger(this)
		elseif(iKey == 34 --[[PageDown]]) then
			this.Input_RTrigger(this)
		elseif(iKey == 87 --[[W]]) then
			this.Input_GeneralUp(this)
		elseif(iKey == 83 --[[S]]) then
			this.Input_GeneralDown(this)
		elseif(iKey == 32 --[[space]]) then
...
function ifs_missionselect_console_fnBuildScreen(this)
	...
	-- BAD_AL Numbers 
	-- on xbox the list heights are about 44% of the screen; description box is about 16% of height
	local list_height = math.floor(h * 0.44)
	local info_height = math.floor(h * 0.16)

	if(gPlatformStr == "PC") then
		if(h > 1000 ) then 
			this.listboxfont = "gamefont_large"
			list_height = math.floor( h * 0.60)
		elseif(h > 800) then
			this.listboxfont = "gamefont_medium"
			list_height = math.floor( h * 0.50)
		elseif(h > 481) then
	...
	elseif(gPlatformStr == "PS2" or gPlatformStr == "PSP") then
		...
		this.listboxfont = "gamefont_medium" -- xbox
	end
	
	local current_font = this.listboxfont
	...
	--ifs_ms_MapList_layout.showcount = math.min(16,math.max(4, math.floor((h - 240) / ListEntryHeight)))
        ifs_ms_MapList_layout.showcount = math.floor( list_height / ((ifs_ms_MapList_layout.yHeight + ifs_ms_MapList_layout.ySpacing) -2) ) 
...
-- 		height = ListHeightC2,
	...
	local BotBoxHeight = info_height --  76
	...
	this.InfoboxBot = NewButtonWindow { 
		...
		Text1 = NewIFText { 
			...
			--font = "gamefont_tiny", 
			font = current_font,
			--textw = ColumnWidthL + ColumnWidthC + ColumnWidthR - 20, old values 
			textw = w * 0.96, -- perfect for xbox 
			texth = BotBoxHeight,
			startdelay=math.random()*0.5, nocreatebackground=1,
